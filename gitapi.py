...
def commits(path):
	...
	i=0
	while i < n_commits:
		resp_json[str(i)]=[repo.commits()[i].id,				# id del commit
				   repo.commits()[i].tree.id,				# id del arbol asociado al commit
				   repo.commits()[i].author.name, 			# nombre del autor del codigo
				   repo.commits()[i].author.email, 			# email del autor del codigo
				   time.asctime(repo.commits()[i].authored_date), 	# fecha de creacion del codigo
				   repo.commits()[i].committer.name, 			# nombre del autor del commit
	  			   repo.commits()[i].committer.email, 			# email del autor del commit
				   time.asctime(repo.commits()[i].committed_date), 	# fecha del commit
				   repo.commits()[i].message] 				# mensaje asociado al commit
		i+=1
